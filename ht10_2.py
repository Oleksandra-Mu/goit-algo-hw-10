...
def monte_carlo_simulation(func, a, b, num_experiments):
    """Виконує серію експериментів методом Монте-Карло."""
    average_area = 0

    for _ in range(num_experiments):
        # Генерація випадкових точок
        points = random.uniform(a, b)
        # Відбір точок, що знаходяться всередині трикутника
        average_area +=func(points)

    return (b-a)*average_area/num_experiments
...
print(f"Теоретична площа: {result}")
